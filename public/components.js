...
(function () {
      ...
      (list || []).forEach((o) => {
        ...
        if (set) {
          ...
          if (set.color !== undefined) {
            ...
            else if (o.fills && o.fills[0]) {
              o.fills[0].kind = "solid";
              o.fills[0].color = set.color;
            } else if (o.strokes && o.strokes[0]) {
              o.strokes[0].kind = "solid";
              o.strokes[0].color = set.color;
            }
          }
        }
        if (o.children) walk(o.children, t);
    ...
    (frame.children || []).forEach((o) => {
      ...
      switch (c.h || "left") {
        case "right":
          ...
          break;
        case "both":
          nw = Math.max(1, b.w + dw);
          break;
        case "center":
          nx = relX + dw / 2;
          break;
        case "scale": {
          const k = prev.w ? frame.w / prev.w : 1;
          nx = relX * k;
          nw = Math.max(1, b.w * k);
          break;
        }
      ...
      switch (c.v || "top") {
        case "bottom":
          ...
          break;
        case "both":
          nh = Math.max(1, b.h + dh);
          break;
        case "center":
          ny = relY + dh / 2;
          break;
        case "scale": {
          const k = prev.h ? frame.h / prev.h : 1;
          ny = relY * k;
          nh = Math.max(1, b.h * k);
          break;
        }
  ...
  function layoutStack(frame, helpers) {
    ...
    const fixedMain = kids.reduce(
      (sum, o, i) => sum + ((o.sizing || "fixed") === "fill" ? 0 : horiz ? boxes[i].w : boxes[i].h),
      0,
    );
    ...
    switch (L.justify || "start") {
      ...
      case "end":
        ...
        break;
      case "between":
        if (kids.length > 1) between = gap + slack / (kids.length - 1);
        break;
    }
  ...
  function hugFrame(frame, helpers) {
    ...
    if (L.mode === "horizontal") {
      frame.w = Math.round(
        pad.l + pad.r + boxes.reduce((s, b) => s + b.w, 0) + gap * (kids.length - 1),
      );
      frame.h = Math.round(pad.t + pad.b + Math.max(...boxes.map((b) => b.h)));
    } else {
      frame.h = Math.round(
        pad.t + pad.b + boxes.reduce((s, b) => s + b.h, 0) + gap * (kids.length - 1),
      );
