...
Axis.prototype.nearest=function(pos,tol){
  const lo=Math.floor((pos-tol)/CELL), hi=Math.floor((pos+tol)/CELL);
  let best=null;
  for(let k=lo;k<=hi;k++){
...
function buildIndex(o){
  const V=new Axis(), H=new Axis();      // V holds x positions, H holds y
  const S=o.settings;
  if(S.artboard){
    const f=o.frame;
  ...
  if(S.guides) (o.guides||[]).forEach(g=>{
  ...
  (o.others||[]).forEach(t=>{
    ...
    if(S.anchors&&t.anchors) t.anchors.forEach(p=>{
      V.add(p.x,{kind:'anchor',obj:t.obj});
      H.add(p.y,{kind:'anchor',obj:t.obj});
    });
  });
  return {V,H,gridStep:o.__gridStep||0,settings:S,others:o.others||[],frame:o.frame};
}
...
function snapAxis(index,axis,probes,tol){
  ...
  probes.forEach(p=>{
    ...
    if(hit&&(!best||hit.d<best.d)) best={d:hit.d, delta:hit.pos-p.pos, line:hit.pos, meta:hit.meta, from:p};
    ...
    if(index.gridStep>0){
      ...
      if(d<=tol&&(!best||d<best.d)) best={d, delta:g-p.pos, line:g, meta:{kind:'grid'}, from:p};
    }
  });
  return best;
}
...
function snapBox(index,box,tol){
  const vp=[{pos:box.x,at:'left'},{pos:box.x+box.w/2,at:'centre'},{pos:box.x+box.w,at:'right'}];
  const hp=[{pos:box.y,at:'top'},{pos:box.y+box.h/2,at:'middle'},{pos:box.y+box.h,at:'bottom'}];
...
function equalGaps(boxes,axis,tolPx,ref){
  ...
  let pool=boxes;
  if(ref){
    pool=boxes.filter(b=>H
      ? (b.y < ref.y+ref.h && b.y+b.h > ref.y)
      : (b.x < ref.x+ref.w && b.x+b.w > ref.x));
  }
