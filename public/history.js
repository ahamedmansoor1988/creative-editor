...
function applyOps(root,ops,dir){
  ...
  list.forEach(op=>{
    const p=op.p, last=p[p.length-1];
...
function nameFor(ops){
  ...
  if(added.length&&added.some(typeOf)) return 'Add '+(typeOf(added.find(typeOf))||'object');
  if(removed.length&&removed.some(typeOf)) return 'Delete '+(typeOf(removed.find(typeOf))||'object');
...
function History(getState,setState){
  this.get=getState; this.set=setState;
...
History.prototype.canUndo=function(){ return this.i>=0; };
History.prototype.canRedo=function(){ return this.i<this.entries.length-1; };
History.prototype.undo=function(){
  if(!this.canUndo()) return false;
  const st=this.get();
  applyOps(st,this.entries[this.i].ops,'from');
  this.i--; this.base=clone(st); this.set(st);
  return true;
};
...
History.prototype.jump=function(target){
  ...
  const st=this.get();
  while(this.i>target){ applyOps(st,this.entries[this.i].ops,'from'); this.i--; }
  while(this.i<target){ applyOps(st,this.entries[this.i+1].ops,'to'); this.i++; }
  this.base=clone(st); this.set(st);
  return true;
};
History.prototype.list=function(){
  return this.entries.map((e,i)=>({i, name:e.name, ops:e.ops.length, current:i===this.i}));
};
History.prototype.reset=function(){
  this.base=clone(this.get()); this.entries=[]; this.i=-1;
};
/** Rough bytes held, for the panel's footer. */
History.prototype.size=function(){
  let n=0;
  this.entries.forEach(e=>{ n+=JSON.stringify(e.ops).length; });
  return n;
};
