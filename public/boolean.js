...
function provide(mod){
  C2=mod;
  waiting.splice(0).forEach(fn=>{ try{ fn(); }catch(e){ console.warn(e); } });
}
function ready(fn){ C2?fn():waiting.push(fn); }
function available(){ return !!C2; }
...
function flattenSubpath(sp,tol){
  ...
  for(let i=0;i<last;i++){
    ...
    if(!a.ox&&!a.oy&&!b.ix&&!b.iy) out.push([b.x,b.y]);   // straight segment
    else flattenCubic(out,a.x,a.y,a.x+a.ox,a.y+a.oy,b.x+b.ix,b.y+b.iy,b.x,b.y,tol);
  }
  return out;
}
...
function compute(op,objs,helpers,fillRule){
  ...
  const toP64=poly=>{
    ...
    poly.forEach(([x,y])=>{
      ...
      if(ix!==px||iy!==py){ p.push(new Point64(ix,iy)); px=ix; py=iy; }
    });
    return p;
  };
  const subj=new Paths64(), clip=new Paths64();
  objs.forEach((o,i)=>{
  ...
  const sol=new Paths64(), open=new Paths64();
  const rule=fillRule==='evenodd'?FillRule.EvenOdd:FillRule.NonZero;
  try{ c.execute(ClipType[OPS[op]||'Union'],rule,sol,open); }
  catch(e){ console.warn('boolean failed:',e&&e.message); return null; }
  const subpaths=[];
  sol.forEach(p=>{
    const pts=[];
    let px=null,py=null;
    p.forEach(pt=>{
      const x=Number(pt.x)/SCALE, y=Number(pt.y)/SCALE;
...
if(window.__clipper2){ provide(window.__clipper2); delete window.__clipper2; }
})();
