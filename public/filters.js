...
function sample(src,w,h,x,y,mode,out,oi){
  let x0,y0;
  if(mode==='wrap'){ x=((x%w)+w)%w; y=((y%h)+h)%h; }
  else if(mode==='mirror'){
    const w2=w*2,h2=h*2;
    x=((x%w2)+w2)%w2; if(x>=w) x=w2-x-1;
    y=((y%h2)+h2)%h2; if(y>=h) y=h2-y-1;
...
function remap(img,w,h,fn,edge){
  ...
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
      const s=fn(x,y);
      const i=(y*w+x)*4;
      if(!s){ dst[i]=dst[i+1]=dst[i+2]=dst[i+3]=0; continue; }
...
function distortion(img,w,h,P){
  ...
  if(!amt) return;
  const cx=w*(0.5+(+P.cx||0)), cy=h*(0.5+(+P.cy||0));
...
function warp(img,w,h,P){
  ...
  remap(img,w,h,(x,y)=>{
    ...
    let u=(x/w)*2-1, v=(y/h)*2-1;
    if(vert){ const t=u; u=v; v=t; }
    let su=u, sv=v;
    switch(env){
      case 'arc':    sv=v-s*(1-u*u); break;
      case 'arch':   sv=v-s*(1-u*u)*(0.5-v*0.5); break;
      case 'bulge':  { const k=1+s*(1-v*v)*0.6; su=u/k; break; }
      case 'flag':   sv=v-s*Math.sin(u*Math.PI*1.5); break;
      case 'wave':   sv=v-s*Math.sin(u*Math.PI*2); break;
      case 'fisheye':{
        const r=Math.hypot(u,v);
        if(r>0.001){ const k=Math.pow(r,1-s*0.8)/r; su=u*k; sv=v*k; }
        break;
      }
    }
    if(vert){ const t=su; su=sv; sv=t; }
...
function displacement(img,w,h,P,mapData){
  const sx=+P.scaleX||0, sy=+P.scaleY||0;
  if(!sx&&!sy) return;
  ...
  const read=(x,y)=>{
    if(mapData){
      const mx=clamp(Math.round(x),0,w-1), my=clamp(Math.round(y),0,h-1);
      const i=(my*w+mx)*4;
...
function haze(img,w,h,P){
  ...
  const lac=+P.lacunarity||2, gain=+P.gain||0.5;
...
function apply(type,cv,params,extra){
  ...
  switch(type){
    case 'distortion':   distortion(img,w,h,params); break;
    case 'warp':         warp(img,w,h,params); break;
    case 'displacement': displacement(img,w,h,params,extra); break;
    case 'haze':         haze(img,w,h,params); break;
    case 'slice':        slice(img,w,h,params); break;
    case 'noise':        noise(img,w,h,params); break;
    default: return cv;
  }
