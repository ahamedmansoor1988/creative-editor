...
describe("Batch 8 geometry effects", () => {
  it("normalizes all three capabilities and preserves stack aliases", () => {
    ...
    expect(o.effects.distortion).toMatchObject({
      mode: "wave",
      amount: 200,
      radius: 0.05,
      edge: "clamp",
    });
    expect(o.effects.warp).toMatchObject({
      envelope: "arc",
      strength: -100,
      axis: "horizontal",
      edge: "clamp",
    });
    expect(o.effects.displacement).toMatchObject({
      scaleX: 300,
      scaleY: -300,
      mapScale: 0.05,
      seed: 99999,
  ...
  it("keeps zero-strength geometry passes out of the active stack", () => {
    ...
    expect(window.FxStack.entryOn({ type: "displacement", params: { scaleX: 0, scaleY: 0 } })).toBe(
      false,
    );
  });
  ...
  it("renders Distortion modes as real pixel remaps", () => {
    for (const mode of ["wave", "twirl", "bulge", "ripple"]) {
      const img = image(24, 24, (x, y) => [x * 10, y * 10, (x + y) * 5, 255]),
        before = [...img.data];
      window.Filters.distortionPixels(
        img,
        24,
        24,
        {
  ...
  it("renders every Warp envelope", () => {
    for (const envelope of window.Filters.ENVELOPES) {
      const img = image(24, 24, (x, y) => [x * 10, y * 10, 80, 255]),
        before = [...img.data];
      window.Filters.warpPixels(
        img,
        24,
        24,
        { envelope, strength: 45, axis: "horizontal", edge: "clamp" },
        {},
      );
  ...
  it("uses procedural displacement when renderer options contain no map", () => {
    ...
    const a = image(24, 24, (x, y) => [x * 10, y * 10, 100, 255]),
      before = [...a.data];
  ...
  it("does not create dark RGB fringes at transformed transparent edges", () => {
    const img = image(32, 32, (x, y) =>
      x >= 8 && x < 24 && y >= 8 && y < 24 ? [255, 30, 20, 255] : [0, 0, 0, 0],
    );
    window.Filters.warpPixels(
      img,
      32,
      32,
      { envelope: "arc", strength: 35, axis: "horizontal", edge: "clamp" },
      {},
    );
    const semis = [];
    for (let i = 0; i < img.data.length; i += 4)
      if (img.data[i + 3] > 0 && img.data[i + 3] < 255) semis.push(img.data[i]);
  ...
  it("offers all three on shapes, text, and images", () => {
    for (const type of ["rect", "text", "image"]) {
      const o = layer(type, {
        ...
        warp: { strength: 10 },
        displacement: { scaleX: 10 },
      });
      expect(editor.FX_PAGES(o)).toEqual(
        expect.arrayContaining(["Distortion", "Warp", "Displacement"]),
      );
    }
  });
});
