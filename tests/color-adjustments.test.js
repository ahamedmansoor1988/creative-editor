      ...
      children: [
        {
          ...
          w: 240,
          h: 180,
          text: type === "text" ? "Color" : undefined,
          fill: { kind: "solid", color: "#6688aa" },
          effects: { colorAdjust },
        },
      ],
    },
...
describe("shared Color Adjustments capability", () => {
  it("normalizes and clamps one reusable parameter object", () => {
    ...
    expect(o.effects.colorAdjust).toEqual({
      exposure: 3,
      blackPoint: 0,
      whitePoint: 1,
      brightness: -1,
  ...
  it("keeps every tonal slider directionally correct", () => {
    ...
    for (const key of ["exposure", "brightness", "highlights", "shadows"]) {
      ...
      expect(
        lum(pixels([level, level, level], { ...neutral, [key]: 0.5 })),
        key + " positive",
  ...
  it("targets highlights and shadows instead of shifting the whole image equally", () => {
    ...
    const hiDark = lum(pixels([35, 35, 35], { ...n, highlights: 0.7 })) - 35;
    const hiBright = lum(pixels([220, 220, 220], { ...n, highlights: 0.7 })) - 220;
    const shDark = lum(pixels([35, 35, 35], { ...n, shadows: 0.7 })) - 35;
    const shBright = lum(pixels([220, 220, 220], { ...n, shadows: 0.7 })) - 220;
  ...
  it("keeps saturation and vibrance neutral on grayscale, while contrast expands tones", () => {
    ...
    expect(lum(pixels([70, 70, 70], { ...n, contrast: 0.5 }))).toBeLessThan(70);
    expect(lum(pixels([190, 190, 190], { ...n, contrast: 0.5 }))).toBeGreaterThan(190);
  });
  ...
  it("moves saturation and vibrance in the expected chroma direction", () => {
    ...
    expect(chroma(pixels(source, { ...n, saturation: 0.5 }))).toBeGreaterThan(chroma(source));
    expect(chroma(pixels(source, { ...n, saturation: -0.5 }))).toBeLessThan(chroma(source));
    expect(chroma(pixels(source, { ...n, vibrance: 0.5 }))).toBeGreaterThan(chroma(source));
    expect(chroma(pixels(source, { ...n, vibrance: -0.5 }))).toBeLessThan(chroma(source));
  });
  ...
  it("makes Vibrance subtle on saturated colors and stronger on muted colors", () => {
    ...
    const chroma = (rgb) => Math.max(...rgb) - Math.min(...rgb);
    const muted = [105, 130, 155],
      saturated = [20, 110, 230];
  ...
  it("supports Figma-style black/white points, brilliance, and gamma", () => {
    ...
    expect(lum(pixels([40, 40, 40], { ...n, blackPoint: 0.25 }))).toBeLessThan(40);
    expect(lum(pixels([210, 210, 210], { ...n, whitePoint: 0.7 }))).toBeGreaterThan(210);
    expect(lum(pixels([110, 110, 110], { ...n, gamma: 2 }))).toBeGreaterThan(110);
    expect(lum(pixels([35, 35, 35], { ...n, brilliance: 0.7 }))).toBeGreaterThan(35);
    expect(lum(pixels([220, 220, 220], { ...n, brilliance: 0.7 }))).toBeLessThan(220);
  });
  ...
  it("supports temperature, tint, and a reusable color filter", () => {
    ...
    expect(pixels([20, 80, 140], { ...n, filterColor: "#ff0000", filterAmount: 1 })).toEqual([
      255, 0, 0,
    ]);
  });
  ...
  it("is available on shapes, text, and images through the same panel", () => {
    for (const type of ["rect", "text", "image"]) {
      const o = layer(type, { contrast: 0.2 });
