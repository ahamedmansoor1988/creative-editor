...
describe("Bloom", () => {
  ...
  it("runs in the pixel pipeline and is available for text", () => {
    const o = shape({ bloom: { amount: 1, radius: 24, threshold: 0.5, knee: 0.2 } }, "text");
    const seen = [];
    const original = window.Filters.apply;
    window.Filters.apply = (type, ...args) => {
      seen.push(type);
  ...
  it("does not feed outer Glow back through Bloom", () => {
    shape({
      ...
      bloom: { amount: 1.4, radius: 24, threshold: 0.65, knee: 0.25 },
    });
    let glowPaints = 0,
      bloomRuns = 0;
    const fill = ctx.fill.bind(ctx),
      apply = window.Filters.apply;
    ctx.fill = (...args) => {
      if (ctx.shadowColor && ctx.shadowColor !== "transparent" && ctx.shadowBlur > 0) glowPaints++;
      return fill(...args);
    };
    window.Filters.apply = (type, ...args) => {
      if (type === "bloom") bloomRuns++;
      return apply(type, ...args);
    };
    try {
      editor.render();
    } finally {
      ctx.fill = fill;
      window.Filters.apply = apply;
    }
...
describe("Background Blur", () => {
  it("normalizes only on vector shapes", () => {
    expect(
      shape({ backgroundBlur: { on: true, radius: 500, opacity: -2 } }).effects.backgroundBlur,
    ).toMatchObject({ on: true, radius: 100, opacity: 0 });
    expect(
      shape({ backgroundBlur: { on: true, radius: 20 } }, "text").effects.backgroundBlur.on,
    ).toBe(false);
  });

  it("reads the existing canvas and clips the softened backdrop to the path", () => {
    shape({ backgroundBlur: { on: true, radius: 24, opacity: 0.8 } }, "ellipse");
    ctx.calls.length = 0;
    editor.render();
