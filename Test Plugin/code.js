...
function linearGradient(stops, deg) {
    return {
...
function radialGradient(stops, scale) {
...
function rangeFor(anchor) {
    ...
    const neg = anchor < 0;
    if (m <= 1)
        return { min: neg ? -1 : 0, max: 1, step: 0.01 };
...
async function pickShader() {
    ...
    for (const s of shaders)
        log(`  - ${s.name} [${s.type}] id=${s.id} imported=${s.imported}`);
    if (shaders.length === 0) {
...
function readShaderProps(node, target) {
    if (target === 'fill') {
        for (const p of fillsOf(node))
            if (p.type === 'SHADER')
                return { ...(p.properties || {}) };
    }
    else {
...
function writeShaderProps(node, target, props) {
    if (target === 'fill') {
        node.fills = fillsOf(node).map((p) => (p.type === 'SHADER' ? { ...p, properties: props } : p));
    }
    else {
        node.effects = node.effects.map((e) => (e.type === 'SHADER' ? { ...e, properties: props } : e));
    }
}
function applyShaderToNode(node, p) {
    const base = [linearGradient(p.shader.stops, p.shader.angle)];
    if (!shaderState) {
        node.fills = base;
        node.effects = [];
        return;
...
function numericParamsForUi(node) {
    if (!shaderState || !node)
        return [];
    ...
    return numericDefs(state.shader).slice(0, MAX_SLIDERS).map(({ defId, def }) => {
        ...
        if (!(defId in state.anchors))
            state.anchors[defId] = value;
...
// Build + apply
// ---------------------------------------------------------------------------
function tag(node, role) {
    node.setPluginData(ROLE_KEY, role);
}
function createNodes(p, at) {
...
function runChecks(n) {
    ...
    const fxTypes = n.highlight.effects.map((e) => e.type);
    ...
    if (shaderState) {
        const applied = shaderState.target === 'fill'
...
function getNodes(frame) {
    const find = (role) => frame.findChild((c) => c.getPluginData(ROLE_KEY) === role);
    ...
    if (!bg || bg.type !== 'RECTANGLE')
        return null;
    if (!blob || blob.type !== 'VECTOR')
        return null;
    if (!shader || shader.type !== 'ELLIPSE')
        return null;
    if (!glass || glass.type !== 'RECTANGLE')
        return null;
    if (!highlight || highlight.type !== 'ELLIPSE')
        return null;
    return { frame, bg, blob, shader, glass, highlight };
}
...
function nextPosition(existing) {
    if (existing)
        return { x: existing.x + FRAME_W + 120, y: existing.y };
    return {
...
// Commands
// ---------------------------------------------------------------------------
async function generate(p, at) {
    params = p;
    if (!shaderState)
        shaderState = await pickShader();
...
async function setShaderParam(defId, value) {
    if (!shaderState || !isFinite(value))
        return;
    const nodes = await currentNodes();
    if (!nodes)
        return;
