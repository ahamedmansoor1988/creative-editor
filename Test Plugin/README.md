...
- `node test/pipeline.js test/cases.json`: the end-to-end pipeline with QC. Each case (reference image, vision, must-haves) runs brief → plan → normalise → match → mock build, then structural QC (everything built, nothing invisible, blur within half a layer's side, features the brief named present, text only on request, budget) and writes `out/`. `--replay` re-runs the saved raw plans through the current normaliser with no AI calls, so normaliser and QC changes iterate in seconds; live cases are spaced 62 s apart for the vision model's limit. `out/report.html` renders each plan approximately beside its reference with a mean colour-error score (≤ 40 good, ≤ 60 fair). Latest: 9/9 pass, mean colour error 33.9.
- `npm test`: 16 offline scenarios (default plan, AI plan with text + glass + named shaders + fallbacks + reference image + tuning, reopen/adopt, broken layer, effect-shader-first, no shaders, no shader API, plan normalisation, glow core and background, solid glows, repeat budget and fan repeats, match reference and recolour, brief reconciliation).
